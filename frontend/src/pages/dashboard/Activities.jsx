...
export default function Activities() {
  ...
  const formatDate = (dateValue, timeValue) => {
    ...
    return `${dateText} at ${formatTime(timeValue)}`;
  };
  ...
  const filteredActivities = useMemo(() => {
    ...
    const nextWeekEnd = endOfDay(addDays(now, 14));
    ...
    const result = activities.filter((activity) => {
      ...
      if (dueDate) {
        if (periodFilter === "overdue") {
          if (activity.status === "completed" || dueDate >= todayStart) {
            return false;
          }
        }
      ...
      const owner = activity.owner?.name?.toLowerCase() || "";
      const ownerEmail = activity.owner?.email?.toLowerCase() || "";
      const leadName = activity.lead?.name?.toLowerCase() || "";
      const organizationName = activity.organization?.name?.toLowerCase() || "";
      const leadEmail = activity.lead?.email?.[0]?.address?.toLowerCase() || "";
      const organizationEmail =
        activity.organization?.email?.toLowerCase() || "";

      const leadPhone = String(
        activity.lead?.phone?.[0]?.number || "",
      ).toLowerCase();
      ...
      const website =
        activity.lead?.website?.toLowerCase() ||
        activity.organization?.website?.toLowerCase() ||
        "";
  ...
  }, [activities, search, typeFilter, periodFilter]);

  // =========================================================
  // GET CONTACT
  // =========================================================

  const getContact = (activity) => {
    if (activity.lead) {
      return {
        type: "lead",
  ...
  const openContact = (activity) => {
    const contact = getContact(activity);
    ...
    if (contact.type === "lead") {
      navigate(`/app/leads/${contact.id}`);

      return;
    }

    if (contact.type === "organization") {
      navigate(`/app/organizations/${contact.id}`);
    }
  };
  ...
  const getEmail = (activity) => {
    return (
      activity.lead?.email?.[0]?.address || activity.organization?.email || ""
    );
  };
  ...
  const getPhone = (activity) => {
    return (
      activity.lead?.phone?.[0]?.number || activity.organization?.phone || ""
    );
  };
  ...
  return (
    <div className="w-full">
        ...
        <div className=" flex flex-col sm:flex-row gap-3 w-full xl:w-auto ">
          ...
          <div className=" relative w-full sm:w-[320px] ">
            ...
            <input
              ...
              className=" w-full border border-gray-300 rounded-xl pl-10
                pr-4 py-2.5 text-sm outline-none bg-white transition focus:border-blue-500 focus:ring-2
                focus:ring-blue-100 "
            />
          </div>
          ...
          <div className="relative">
            <select
              ...
              className=" appearance-none
                w-full sm:w-[190px] border border-gray-300 rounded-xl px-4 py-2.5 pr-10 text-sm bg-white outline-none
                cursor-pointer focus:border-blue-500 focus:ring-2 focus:ring-blue-100 "
            >
      ...
      <div className=" flex items-center gap-6 overflow-x-auto border-b border-gray-200 mb-5 ">
        {[
          ["todo", "To-do"],
          ["overdue", "Overdue"],
          ["today", "Today"],
          ...
          <button
            ...
            className={` whitespace-nowrap py-3
                text-sm font-medium border-b-2 -mb-px transition
                ${
                  periodFilter === value
        ...
        <div className=" overflow-x-auto ">
          <table className=" w-full min-w-[1100px] text-sm ">
            ...
            <thead className=" bg-gray-50 border-b border-gray-200 ">
              <tr>
                <th className=" text-left px-4 py-3 font-semibold text-gray-600 w-[70px] ">
                  Done
                </th>
                    ...
                    >
                      ...
                      >
                        {activity.organization ? (
                          <button
                            ...
                            className=" flex items-center gap-2 truncate text-blue-600 hover:underline text-left "
                          >
                            <Building2 size={15} />
                      ...
                      <td className=" px-4 py-4 ">
                        {email ? (
                          <a
                            ...
                            className=" flex items-center gap-2 text-blue-600 hover:underline max-w-[220px] "
                            title={email}
                          >
                            <Mail
                              size={15}
                      ...
                      <td className=" px-4 py-4 whitespace-nowrap ">
                        <div className=" flex items-center gap-2 text-gray-700 ">
                          ...
                          {formatDate(activity.dueDate, activity.startTime)}
                        </div>
                      </td>
